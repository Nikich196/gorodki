...
public sealed class TerritoryRestoreTests
{
    ...
    public void Victims_visit_to_the_rest_of_a_transferred_piece_leaves_a_seam_in_the_fallback()
    {
        ...
        // весь квадрат. В проекции это закрыл точный откат (ExactUndo): там визит остатка ложится на весь прежний
        // квадрат. Этот тест — предел отката по граням (запасной путь); упадёт — значит, откат по граням научился
        // большему.
        var map = new TerritoryMap();
        Capture(map, Anna, T0, RectanglePolygon(100, 100, 200, 200));
    ...
    // В проекции шов закрыл точный откат по строкам журнала (ExactUndo: времена со всех вставленных кусков над прежним;
    // ExactUndoTests, TerritoryTests); эти тесты — предел отката по граням, запасного пути проекции.
