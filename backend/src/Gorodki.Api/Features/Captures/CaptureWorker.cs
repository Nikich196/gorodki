    ...
    : BackgroundService
{
    ...
    /// <summary>
    /// Раз в час: стирается журнал захватов старше недели (строки точного отката — через час-два, когда захват уже не может
    /// быть скрыт), сырые точки старше 14 дней и истёкшие токены входа, закрываются
