    ...
    ILogger<CaptureProcessor> logger)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // В этих тайлах нетронутая земля разрезана границей петли (аудит BE-01): пока захват скрыт, проекция вернёт
            // прежние строки, но после раскрытия разрез останется на карте. Движок не ошибся — пишем только частоту,
            // без тайлов и координат: сам лог не должен выдавать, где бегали.
            logger.LogWarning(
