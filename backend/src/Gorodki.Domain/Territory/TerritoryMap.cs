...
/// разрезана границей петли. Пока захват скрыт, проекция возвращает прежние строки (точный откат), но после раскрытия
/// разрез останется на карте. Сервер пишет это в лог — иначе частоту не узнать.
/// </param>
...
public sealed class TerritoryMap(TerritoryRules rules, SliverSettings slivers)
{
    ...
    /// (со своим временем визита или без визита) с возвращённой землёй не сливается, — поэтому публичная проекция
    /// откатывает скрытый захват точно, по строкам журнала (<see cref="ExactUndo"/>), а сюда идёт запасным путём.
    /// Освежение владельцем своей земли его собственным захватом — тот же <see cref="CaptureRules.Visit"/>, от визита
    /// не отличить, и переносится так же.
