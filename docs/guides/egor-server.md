...
заготовки контрактов (сделано 26.09: [раздел 7.0](#70-заготовки-контрактов-где-что)) · C5 каркас Hangfire · C18 инструмент калибровки · C3 большая петля (сделано 25.09) · C6 конвейер
...
до этого задачи на GitHub ещё нет. Заготовки волн 1–4 (кроме E6, E18–E21, E23) сделаны 26.09 — задачи #135–#148, что где —
[раздел 7.0](#70-заготовки-контрактов-где-что). **Ворота** — правило «за 3 дня до срока» (раздел 1).
...
| 1 | E5a–c **ворота** | Кланы: создать и вступить → выход, роли, лидерство → переименование, оттенки | 10.11 | — (решения по [#64](https://github.com/Nikich196/gorodki/issues/64) приняты 25.09: PLAN §3.3, §3.6) | [#135](https://github.com/Nikich196/gorodki/issues/135) — заготовка готова |
| 2 | E7 **ворота** | Суточный срез очков и рейтинг территории | 13.11 | C9 (очки за захват), C4 (смена сезона) — сделаны 25.09 | [#136](https://github.com/Nikich196/gorodki/issues/136) — заготовка готова |
| 2 | E9 **ворота** | % Бреста и районов в `/fog/summary` | 15.11 | C6 (конвейер OSM) | [#138](https://github.com/Nikich196/gorodki/issues/138) — заготовка готова |
| 2 | E12 | Карточка недели `GET /me/weekly` | 30.11 | E9 (для %) | [#141](https://github.com/Nikich196/gorodki/issues/141) — заготовка готова |
| 3 | E11 | «Припасы» и Рюкзак | 30.11 | — | [#140](https://github.com/Nikich196/gorodki/issues/140) — заготовка готова |
| 3 | E22 | Серия и «Заморозка серии» | 30.11 | — | [#148](https://github.com/Nikich196/gorodki/issues/148) — заготовка готова |
| 3 | E10 | «Входящие» и бюджет уведомлений | 30.11 | C10 (события после границы) | [#139](https://github.com/Nikich196/gorodki/issues/139) — заготовка готова |
| 3 | E13 | Оценка доверия, подозрительные забеги, бан | 30.11 | — | [#142](https://github.com/Nikich196/gorodki/issues/142) — заготовка готова |
| 3 | E14a, E14b | Друзья по коду; лента | 30.11 | помощник границы (C11) для ленты | [#143](https://github.com/Nikich196/gorodki/issues/143), [#144](https://github.com/Nikich196/gorodki/issues/144) — заготовки готовы |
| 3 | E15 | «Коллекция» и админка тайников | 30.11 | C12 (тайники) | [#145](https://github.com/Nikich196/gorodki/issues/145) — заготовка готова |
| 3 | E8 | Зал славы | 30.11 | E7 | [#137](https://github.com/Nikich196/gorodki/issues/137) — заготовка готова |
| 3 | E23 | Бот-симулятор (20 ботов × 30 дней, CSV) | 30.11 | — | после заготовки |
| 4 | E17 | Дуэли | 08.12 | C5 | [#147](https://github.com/Nikich196/gorodki/issues/147) — заготовка готова |
| 4 | E16 | «Короли участков» | 08.12 | C16 (отрезки) | [#146](https://github.com/Nikich196/gorodki/issues/146) — заготовка готова |
...
со `Skip`) делает Claude перед стартом** — тогда появляется задача на GitHub с подробностями. Правила игры — в
[PLAN.md](../PLAN.md), ссылки на разделы — в карточках.

### 7.0 Заготовки контрактов: где что

Сделаны 26.09 (C15). В каждой задаче — ответы и запросы (DTO), адреса-заглушки `// ЗАДАЧА #NN` (бросают
`NotImplementedException` — это 500, пока задача не сделана), образцы ответов в `contracts/samples` (по ним телефон уже
строит экраны, Swift-тесты их разбирают), интеграционные тесты со `Skip = "ЗАДАЧА #NN"` и строки адресов с `{id}` в
`IdorTests.AwaitingTasks`. Описание API (`contracts/openapi.v1.json`) уже содержит все адреса — **имена, поля и типы не
меняй**, только тело метода; нужна правка — согласуй в задаче (раздел 2.5). Коды ошибок каждого адреса — в его
`WithDescription`. Всё в `backend/src/Gorodki.Api/Features/`:

| Задача | Адреса | Файл заглушки | Образцы | Тесты (`backend/tests/Gorodki.IntegrationTests/`) |
|---|---|---|---|---|
| E5a–c кланы [#135](https://github.com/Nikich196/gorodki/issues/135) | `POST /clans`, `GET /clans/mine`, `GET /clans/{id}`, `POST /clans/join`, `POST /clans/mine/leave`, `DELETE /clans/mine/members/{userId}`, `PUT /clans/mine/members/{userId}/role`, `PUT /clans/mine/name`, `POST /clans/mine/code`, `GET /clans/hues` | `Clans/ClanEndpoints.cs` | `clan`, `clan-mine`, `clan-hues` | `ClansTests` (7) |
| E7 рейтинг территории [#136](https://github.com/Nikich196/gorodki/issues/136) | `GET /leaderboards/territory?league=&season=`; конфиг `scoring.holdTiers`, `scoring.holdCapSotki` | `Leaderboards/LeaderboardEndpoints.cs` | `leaderboard-territory` | `TerritoryLeaderboardTests` (3) + тесты среза — свои |
| E8 Зал славы [#137](https://github.com/Nikich196/gorodki/issues/137) | `GET /hall-of-fame` | `HallOfFame/HallOfFameEndpoints.cs` | `hall-of-fame` | `HallOfFameTests` (1) + тесты снимка — свои |
| E9 % Бреста [#138](https://github.com/Nikich196/gorodki/issues/138) | `GET /fog/summary`: `brestPercent`, `districts`, `osmSetVersion` (необязательные) | `Fog/FogEndpoints.cs` | `fog-summary` | `FogPercentTests` (1 + контракт без `Skip`) |
| E10 «Входящие» [#139](https://github.com/Nikich196/gorodki/issues/139) | `GET /inbox?cursor=`, `POST /inbox/read` | `Inbox/InboxEndpoints.cs` | `inbox` | `InboxTests` (3; один ждёт и C10) |
| E11 Рюкзак [#140](https://github.com/Nikich196/gorodki/issues/140) | `GET /me/inventory`, `POST /me/inventory/{id}/activate` | `Inventory/InventoryEndpoints.cs` | `inventory` | `InventoryTests` (3) |
| E12 карточка недели [#141](https://github.com/Nikich196/gorodki/issues/141) | `GET /me/weekly?week=` | `Me/WeeklyCardEndpoints.cs` | `weekly` | `WeeklyCardTests` (3) |
| E13 доверие и бан [#142](https://github.com/Nikich196/gorodki/issues/142) | `GET /admin/runs/suspicious`, `POST /admin/users/{userId}/ban` (вне описания API) | `Admin/TrustEndpoints.cs` | — | `AdminTrustTests` (2) |
| E14a друзья [#143](https://github.com/Nikich196/gorodki/issues/143) | `GET /friends`, `POST /friends`, `POST /friends/{id}/accept`, `DELETE /friends/{id}` | `Social/FriendEndpoints.cs` | `friends` | `FriendsTests` (3) |
| E14b лента [#144](https://github.com/Nikich196/gorodki/issues/144) | `GET /feed?scope=friends\|all&cursor=`, `POST /feed/{id}/respect`, `POST /feed/{id}/report`, `GET /me/blocks`, `PUT/DELETE /me/blocks/{id}` | `Social/FeedEndpoints.cs` | `feed`, `player` | `FeedTests` (3) |
| E15 «Коллекция» [#145](https://github.com/Nikich196/gorodki/issues/145) | `GET /collection` | `Collection/CollectionEndpoints.cs` | `collection` | `CollectionTests` (1) |
| E16 «Короли участков» [#146](https://github.com/Nikich196/gorodki/issues/146) | `GET /segments?league=`, `GET /segments/{id}`, `GET /segments/{id}/leaderboard?league=&season=` | `Segments/SegmentEndpoints.cs` | `segments`, `segment`, `segment-leaderboard` | `SegmentsTests` (2) |
| E17 дуэли [#147](https://github.com/Nikich196/gorodki/issues/147) | `GET /duels`, `POST /duels`, `GET /duels/{id}`, `POST /duels/{id}/accept`, `POST /duels/{id}/decline` | `Duels/DuelEndpoints.cs` | `duels` | `DuelsTests` (4) |
| E22 серия [#148](https://github.com/Nikich196/gorodki/issues/148) | `GET /me/streak` | `Streaks/StreakEndpoints.cs` | `streak` | `StreakTests` (2) |

**Граница публичности в заготовках** (раздел 4) — самое важное, что проверяют тесты: пост ленты, событие «Входящих», счёт
дуэли, проход отрезка, очки рейтинга и Зала славы о чужих действиях появляются **не раньше, чем их покажет карта**; в ответах
нет координат и времени чужих действий (у поста — только дата, у прохода — только длительность, у события — момент, когда
оно стало видно). Номера постов и событий — `Guid.NewGuid()`, а не `Guid.CreateVersion7()`: в v7 зашито время создания до
миллисекунды, то есть время захвата. Ники — всюду по правилу карточки игрока (#115).

Чего в заготовках нет: E6, E18–E21, E23 (их заготовки — позже); админки тайников и отрезков (после C12 и C16); чистых функций
(`ClanRules`, `NotificationBudget`, `TrustSignals`, `SegmentEffortDetector`, правило серии) — их и тесты к ним без Docker
пишешь сам, как в карточках ниже.
...
Кланы в движке земли и на карте — задача Claude (C8). **Заготовка готова: [#135](https://github.com/Nikich196/gorodki/issues/135)**
(адреса — в [7.0](#70-заготовки-контрактов-где-что); передача лидерства — только при выходе лидера, отдельного адреса нет).
...
`holdTiers` и `holdCapSotki` добавлены заготовкой, значения предварительные); строка удержания в книге очков (новый вид `Hold` — поле `kind`, проверка `ck_score_events_kind` и
...
`ScoreBook.FinalTotalsAsync`, один раз, повтор ничего не меняет. **Заготовка готова: [#136](https://github.com/Nikich196/gorodki/issues/136)**
(в ответе `final` — «итог», иначе «предварительно»).
...
*Что делаешь ты:* новые поля ответа — заготовка готова, **[#138](https://github.com/Nikich196/gorodki/issues/138)**: у слоя
`brestPercent` и `districts`, у ответа `osmSetVersion` (номер набора рядом с процентом, чтобы телефон мог сказать
«пересчитано по новой карте»); читать только свои тайлы (процент — персональные данные, как весь туман).
...
тумана (из `FogNewCells` забегов), % Бреста, площадь своих захватов («взятое» из заявок — его автор видит сразу); только числа. **Заготовка готова:
[#141](https://github.com/Nikich196/gorodki/issues/141)** (`week` — понедельник, без него — прошлая неделя).
...
`POST /me/inventory/{id}/activate` (нельзя во время активного забега). **Заготовка готова:
[#140](https://github.com/Nikich196/gorodki/issues/140).**
...
считается днём серии, решила заготовка ([#148](https://github.com/Nikich196/gorodki/issues/148)): игровые сутки по Минску с
хотя бы одним завершённым живым забегом с засчитанным путём; порог пути и как тратится «Заморозка» — уточнит Никита.
...
**Заготовка готова: [#139](https://github.com/Nikich196/gorodki/issues/139)** (`POST /inbox/read {upToAtMs}`).

**E13 — оценка доверия** (PLAN §3.9). Волна 3, 30.11. `TrustSignals` — чистая функция по признакам забега, флаги забега,
`GET /admin/runs/suspicious`, `POST /admin/users/{id}/ban`. **Заготовка готова:
[#142](https://github.com/Nikich196/gorodki/issues/142)** (адреса вне описания API).
...
респекты, жалобы, блокировки, `GET /feed?scope=`. Комментариев нет. **Заготовки готовы:
[#143](https://github.com/Nikich196/gorodki/issues/143) (друзья), [#144](https://github.com/Nikich196/gorodki/issues/144)
(лента: `scope=friends|all`, блокировки — `/me/blocks`).**

**E15 — «Коллекция»** (PLAN §3.12). Волна 3, 30.11, после C12 (тайники в конвейере). `GET /collection` — **без координат**,
тест обязан это проверять; админка тайников. **Заготовка готова: [#145](https://github.com/Nikich196/gorodki/issues/145)**
(админка тайников — после C12).
...
([итог сезона](../architecture/scoring-and-seasons.md#итог-сезона--момент-закрытия)). **Заготовка готова:
[#137](https://github.com/Nikich196/gorodki/issues/137)** (значение топ-клана — уточнит Никита).
...
неделю; итог — задача Hangfire; ставки — одной транзакцией. **Заготовка готова:
[#147](https://github.com/Nikich196/gorodki/issues/147)** (живой счёт — по границе публичности).
...
админка отрезков. **Заготовка готова: [#146](https://github.com/Nikich196/gorodki/issues/146)** (админка отрезков — после C16).
