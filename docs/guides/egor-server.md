...
1. **Граница публичности.** Чужой захват другие видят только через 20 минут, пачками по 5 минут (`TerritoryReader.PublicHorizon`,
   ...
   - **очки сезона** (рейтинги, срез E7, Зал славы) — только через `ScoreBook.Visible(db, now)` (`visible_at ≤ now`) или
     `ScoreBook.SeasonTotalsAsync`: очки за захват видны с той же границы, что и сам захват на карте, — даже самому автору
...
- **Очки сезона** — `ScoreBook.SeasonTotalsAsync(db, лига, сезон, now, ct)`: сумма уже видимых начислений по игрокам.
  Любое своё чтение книги — только через `ScoreBook.Visible(db, now)` (раздел 4).
...
`GET /leaderboards/territory?league=&season=` — копия `GetExploration`. Образец — `LeaderboardSnapshots`.
...
значение. При удалении аккаунта — обезличить (игрок → пусто), а не удалять строку. Опора: очки сезона —
`ScoreBook.SeasonTotalsAsync` (только видимые), закончился ли сезон и прошла ли смена — `seasons.reset_at` следующего
сезона ([очки и сезоны](../architecture/scoring-and-seasons.md)); снимок — один раз на сезон, повтор ничего не меняет.
