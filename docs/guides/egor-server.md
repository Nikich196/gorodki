...
за захват · C4 смена сезона · C7 «Ничейные земли» · C8 кланы в движке и на карте · C10 события уведомлений после
...
1. **Граница публичности.** Чужой захват другие видят только через 20 минут, пачками по 5 минут (`TerritoryReader.PublicHorizon`,
   ...
   - никогда не считай ничего по **настоящим** кускам земли (`db.Parcels`) для ответа игроку — ни «площадь моей земли», ни
     ...
     бы его. Такие числа — только через помощник `TerritoryReader.VisibleOwnedAreaAsync(userId, league, ct)` (C11): площадь
     своей земли в м² такой, какой её видят остальные (та же проекция, что у карты; угасшая не считается; свой свежий
     захват прибавится через те же 20–25 минут);
   - события о чужих действиях (лента, «Входящие», уведомления) получают `visible_at` через помощник
     `TerritoryReader.VisibleAtAsync(actorId, appliedAt, ct)` — не раньше границы. `appliedAt` — время **применения**
     к карте (`captures.applied_at`, `runs.visits_processed_at`), а не петли: петля из офлайна применяется позже;
...
- **Не добавляй площадь своей земли** в эту задачу (раздел 4, пункт 1). Тест проверяет, что полей в ответе ровно шесть.
  Посчитать её безопасно теперь можно (`TerritoryReader.VisibleOwnedAreaAsync`, C11), но это новое поле контракта с
  телефоном: оно добавляется отдельным шагом после E4 — заготовку (поле, пример, копию для телефона) делает Claude.
