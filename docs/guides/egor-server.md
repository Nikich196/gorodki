...
Лучше сделать меньше, но понимать каждую строку. Правило «задача до 1,5 часа» из [CONTRIBUTING.md](../../CONTRIBUTING.md#задачи-для-друга)
...
заготовки контрактов · C5 каркас Hangfire · C18 инструмент калибровки · C3 большая петля · C6 конвейер OSM · C11 визиты
в ответе забега и помощник границы публичности · C14 линии границ · C2 #101 · C9 очки SP за захват · C4 смена сезона ·
C7 «Ничейные земли» · C8 кланы в движке и на карте · C10 события уведомлений после границы · C13 «Радар»/«Магнит» ·
C12 тайники в конвейере · C16 отрезки и вело · C17 демо-режим.
...
│  │  ├─ Territory/          участки: правила захвата, угасание, визиты, движок TerritoryMap
...
1. **Граница публичности.** Чужой захват другие видят только через 20 минут, пачками по 5 минут (`TerritoryReader.PublicHorizon`,
   ...
   - никогда не считай ничего по **настоящим** кускам земли (`db.Parcels`) для ответа игроку — ни «площадь моей земли», ни
     ...
     бы его. Такие числа Claude даст через помощник (`TerritoryReader.VisibleOwnedArea`, задача C11);
   - события о чужих действиях (лента, «Входящие», уведомления) получают `visible_at` через помощник Claude — не раньше
     границы;
...
| 1 | E5a–c **ворота** | Кланы: создать и вступить → выход, роли, лидерство → переименование, оттенки | 10.11 | решения по [#64](https://github.com/Nikich196/gorodki/issues/64) | после заготовки |
| 2 | E7 **ворота** | Суточный срез очков и рейтинг территории | 13.11 | C9 (очки за захват), C4 (смена сезона) | после заготовки |
...
2. **Выпуск** (`CreateInvites`): админ? → проверка пределов → коды → сохранить → `201`.
   ...
   - Ответ: `TypedResults.Created("/admin/invites", список)`. Перевод времени: `DateTimeOffset.FromUnixTimeMilliseconds(ms)`
     и обратно `.ToUnixTimeMilliseconds()` — удобно сделать маленький `ToResponse(InviteEntity)`.
...
`AwaitingTasks`, в `auth.md` раздел «Инвайты», CI зелёный, в PR 5 строк. Около 3–4 часов, удобно в два захода: 1 — код,
2–4 — адреса.
...
**Подводные камни.**
- **Не добавляй площадь своей земли**, даже если очень хочется (раздел 4, пункт 1). Тест проверяет, что полей в ответе ровно
  шесть.
...
**E6 — часовые чистки в Hangfire.** Волна 1, октябрь, после C5 (каркас Hangfire: `Hangfire.PostgreSql`, схема `hangfire`,
выключатель для тестов, одна задача-образец). Цель: то, что сейчас раз в час делает `CaptureWorker.HousekeepingAsync`
(стирание старого журнала захватов, закрытие забытых забегов, стирание сырых точек и истёкших токенов, удаление аккаунтов,
ежедневный срез рейтингов), — отдельными повторяющимися задачами Hangfire по образцу C5. Тесты: каждая
задача вызывается напрямую и делает то же, что раньше; фоновый обработчик в тестах выключен.
...
**E5a–c — кланы** ([#64](https://github.com/Nikich196/gorodki/issues/64), PLAN §3.6). Волна 1, срок 10.11, **ворота**.
- a — `ClanEntity` (имя, нормализованное имя, оттенок, код, лидер, сезон переименования), `ClanMemberEntity` (роль, дата),
  у игрока «можно вступать с»; `POST /clans`, `GET /clans/mine`, `GET /clans/{id}`, `POST /clans/join {code}`;
- b — выход, исключение, роли, передача лидерства (офицеру, иначе самому давнему); 72 ч без вступления и вышедшему,
  и исключённому; меньше 3 человек — клан «неполный»;
- c — переименование (лидер, раз в сезон), `GET /clans/hues`, новый код.
...
**E7 — суточный срез очков и рейтинг территории** (PLAN §3.5). Волна 2, 13.11, **ворота**, после C9 (очки за захват) и C4
(смена сезона). Срез в 00:00 по Минску: удержание — ступени по площади «касались в сезоне», с потолком; дистанция +10 за км до
20 км в сутки, «Вело» ×0,33; книга очков с ключом `(день, игрок, вид)` — повтор среза не начисляет дважды.
`LeaderboardBoard.Territory` и `GET /leaderboards/territory?league=&season=` — копия `GetExploration`. Образец —
...
значение. При удалении аккаунта — обезличить (игрок → пусто), а не удалять строку.
