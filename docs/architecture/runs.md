...
- **Номера точек** — подряд с нуля. Точку с отрицательной точностью телефон выбрасывает **до** нумерации;
  отрицательная скорость → `null`. `flags`: 1 — координаты подставлены программой, 2 — от внешнего устройства.
