...
в CI вместе с GameCore. Хранилище на GRDB и запуск по расписанию (BGTasks) — в приложении, за протоколом `SyncStore`
(PLAN.md, §7.2: актор `SyncEngine`, outbox). Как движок получает клиент API с подписью запросов и игрока — в
[ios-app.md](ios-app.md) (`AppDependencies.syncEngine()`; очередь пока в памяти).
