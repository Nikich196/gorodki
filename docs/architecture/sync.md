...
- **Второй «Старт»** — пока первый начинается или забег идёт — отклоняется до записи в базу.
- **Разрешения — до записи забега**: «При использовании» и точная геопозиция (приблизительная не пройдёт судью, а пустой
  забег съест суточный лимит). Датчики движения — по статусу CoreMotion (`motionAuthorized` в забеге).
...
в продолженный забег, а если продолжать нечего — выбрасывается. Датчики дозапрашиваются с отметки уже отправленных
(`acceptsSensorsAfter`): CoreMotion отдаёт историю за время выгрузки.

**Выход из аккаунта** — сначала «Финиш», потом стереть вход (`RunController.signOut`): забег принадлежит игроку, а
событие о смене входа приходит уже после стирания. Истёкший вход запись не останавливает — очередь дождётся входа.
...
отбросились бы как запоздавшие. Геопозиция и датчики во время повтора не запускаются. Остановка — забег заканчивается
на достигнутом времени записи. Повтор разрешён только ролям `demo` и `admin` — сервер отвечает 403 `replay_forbidden`.
