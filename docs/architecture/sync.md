...
Не сделано: экран забега (дизайн — Никита), временная точная геопозиция (ключ в Info.plist и запрос — вместе с экраном),
замер запаздывания датчиков (S1). Проверить на телефоне: забег в кармане 30 минут, выгрузка приложения из фона и
