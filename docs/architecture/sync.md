...
продолжаются и закрываются отдельно (`recover(labProbe: true)`): продолжение одного вида не трогает забеги другого. Флаг в UserDefaults — только подсказка «забег мог идти»: по нему `AppDelegate` синхронно, ещё до чтения
базы, запускает геопозицию (иначе iOS может снова усыпить приложение); поступившее копится в очереди трекера и уходит
в продолженный забег, а если продолжать нечего — выбрасывается, а Live Activity прежнего процесса закрывается.
