...
| Сервер попросил дослать точки, забыл забег (404) или ещё не видит завершения | 15 с (число кругов ограничивает `SyncEngine`) |
...
- **Нечитаемая запись** (файл повреждён или модель изменилась так, что тест выше этого не поймал) пропускается при
  чтении и остаётся в таблице; на iPhone о ней пишется строка в системный журнал (таблица и ключ). Раньше одна такая
  строка роняла всю очередь навсегда: «Старт» (новичок ли игрок), продолжение забега и каждый проход синхронизации.
  Версия приложения, которая её прочтёт, ещё сможет её доставить.
...
- Проверено: один и тот же набор тестов поведения — для очереди в памяти и в базе; очередь переживает закрытие
  ...
  отдельно» этот тест роняет); замороженный JSON версии 1 читается; нечитаемая строка пропускается и не стирается;
