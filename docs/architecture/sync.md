...
- **Демо-повтор** (4 теста): запись — время от старта, без абсолютной даты, переживает JSON; повтор ×20 — время сдвинуто
  ...
  достигнутом времени.
- **Исправления по ревью трекера** (6 тестов): поступившее во время «Старта» второго забега — в него, по порядку;
  неудачный «Финиш» — забег продолжается; ошибка записи не гаснет от датчиков; повтор не продолжается после
  перезапуска; датчики после перезапуска — не раньше окна; запоздавший датчик в записи — после пришедшего раньше.
