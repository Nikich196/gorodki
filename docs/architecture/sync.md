...
`takeRecording` после «Финиша»): всё поступившее в трекер со временем от старта, без абсолютной даты; запись одна,
последняя, лежит только на телефоне (`Application Support/demo-recording.json`, настоящий маршрут — в репозиторий не
кладётся). `RunReplay` проигрывает её в новый забег `source = replay`: начало = «сейчас» − длина записи (при ×20 точки
