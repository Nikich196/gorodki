...
erDiagram
    ...
    capture_journal ||--o{ capture_journal_parcels : "строки кусков для точного отката"
...
| `capture_journal_parcels` | Строки `parcels`, которые захват удалил и вставил в тайле, — для точного отката в публичной проекции ([карта](territory-map.md)) | `replaced` — удалённая строка, с контуром в TWKB как лежал (`replaced = (geometry IS NOT NULL)`), иначе вставленная; `parcel_id` — номер строки без внешнего ключа; состояние целиком; пишется вторым сохранением в транзакции захвата (лучшее усилие — без строк проекция идёт запасным путём); хранится 3–4 часа; строки удалённого аккаунта стираются вместе с ним, его номер — из `loss_attackers` |
...
| `seasons` | Календарь сезонов (PLAN.md, §3.4) | Номер как в плане (С0 = 0), начало — полночь по Минску (другое база не примет: `ck_seasons_minsk_midnight`), хранится в UTC; конец — начало следующего; `GET /seasons` для приложения. «За всё время» у сезонных данных — −1 |
...
## Строка куска на месте не меняется

Инвариант, на котором стоит точный откат скрытого захвата (`ExactUndo`, [карта](territory-map.md)): контур строки
`parcels` после вставки не меняется никогда. Захват и откат нарушителя пишут только разницу — удаляют строки и вставляют
новые (`ParcelDiff`); на месте строку меняют только двое:

- **визиты** (`VisitProcessor`) — `level`, `last_visit_at`, `last_level_up_at` (`CaptureRules.Visit`; точный откат узнаёт
  их по `VisitReplay.Trace`);
- **удаление аккаунта** (`AccountDeletion`) — номер удалённого из `loss_attackers` (так же чистятся строки
  `capture_journal_parcels`, и обмен строк остаётся согласованным).

Всё остальное, что захочет править строку на месте (смена сезона, разовая миграция кусков), сначала должно решить, как
его увидит проекция скрытого захвата. Архитектурный тест `ParcelWritersTests` ищет в коде сервера и миграциях
`UPDATE app.parcels`, `ExecuteUpdate` по `Parcels` или `Set<ParcelEntity>()` (и по запросу к ним, сохранённому в
переменную), `Update/Attach`, присваивание полям куска (и составное: `+=`, `??=`, `++`) и — в файлах, где есть куски, —
правку через трекер изменений (`Entry(…)`, `SetValues`, `CurrentValue =`) вне этих двух файлов. Это поиск по тексту: он
ловит обычные формы, а не любую правку, — новый писатель строк кусков всё равно проверяется ревью.
...
- `ParcelWritersTests` — строки `parcels` на месте правят только визиты и удаление аккаунта.
...
строки точного отката (`capture_journal_parcels`) — 3–4 часа (`max(3 ч, 2 × (задержка + шаг раскрытия))`, чистка раз в
час; три часа — запас на подъём задержки в конфиге, [захваты](captures.md); строка ~230 Б плюс ~5 Б на вершину удалённого
контура: обычно 2–4 МБ, в худшем случае — все захваты тяжёлые — ~80 МБ), туман — сжатые биты,
токены входа — до конца срока (30 дней; каждое обновление добавляет строку, истёкшие стираются раз в час).

`GET /health/ready` (без входа; его вызывает пингер) показывает общий статус и статус каждой проверки (`database`, `storage`).
Бюджет — `databaseBytes`/`databaseMegabytes`, `connections`, `parcels`, `vertices`, `captureJournalParcelsBytes` — и описания проверок видит только
