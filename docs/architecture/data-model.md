...
| `invites` | Инвайт-коды закрытой регистрации | Число использований защищено от гонки (`xmin`) |
| `game_configs` | Версии игрового конфига (все числа правил) | Забег проверяется той версией, с которой начат |
| `runs` | Забеги | `league` с первого дня (D16); `processed_seq` — до какой точки обработан |
| `run_chunks` | Точки забега кусками | Повтор того же содержимого не сохраняется (уникальный хэш) |
...
Следующие таблицы (вход и токены, устройства, кланы, лента, туман, фишки, тайники…) добавляются миграциями по ходу этапов (PLAN.md, §7.3).
