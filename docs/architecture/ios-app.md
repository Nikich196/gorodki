...
- **Земля** (`GameMapView`, MKMapView как у стенда S4): один `MKMultiPolygon` на стиль (отношение, цвет, уровень) без
  обводки и без сглаживания краёв (`LandFillRenderer`): со сглаживанием общий край соседних кусков закрашивался дважды
  наполовину, и по линии тайла была видна светлая нить (снимок 24 первого прогона); внешний край закрыт кромкой.
  Картинка зависит от `MapModel.renderVersion` — экран читает его в `body` и передаёт карте: сам `updateUIView` за
  чтениями модели не следит. Стиль — `LandStyle`: отношение
