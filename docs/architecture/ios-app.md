...
- **Вкладки** (`App/Shell`): системный `TabView` — Карта · Рейтинги · Клан · Профиль («Поиск» убран, PLAN.md, §5 и §15).
  ...
  единственный цветной элемент управления. Земли, туман, HUD — следующие шаги; «Рейтинги» и «Клан» пока заглушки.
  «Профиль»: ник, цвет, туман за всё время и за сезон (`GET /me`, `/seasons`, `/fog/summary`; при входе, «потянуть
  вниз» и при заходе на вкладку, если в прошлый раз не загрузилось), «Отладка», «Выйти». Выход — только после вопроса:
...
| Аргумент | Значения |
|---|---|
| `-GorodkiScreen` | `intro`, `invite`, `age`, `consent`, `sign-in`, `map`, `leaderboards`, `clan`, `profile`, `debug` |
...
`ProfileModel`, `ShellModel`) — `@Observable` с простыми значениями: фикстура просто задаёт их.
