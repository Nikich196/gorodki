...
- **Live Activity.** У забега, прогулки «Лаборатории» и проверки установки один тип атрибутов, поэтому каждый хранит
  ...
  закончившийся по пределу длины в фоне, может не успеть закрыть плашку до того, как iOS усыпит приложение, — тогда её
  закроет `RunController` при следующем запуске.
