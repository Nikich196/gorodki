...
- **Онбординг** (`App/Onboarding`, PLAN.md, §3.18): три карточки интро → код приглашения (заглавными, без пробелов по
  краям — `InviteCode`) → «Мне 16 лет или больше» → «Принимаю пользовательское соглашение» и отдельно «Даю согласие…»
  под текстом согласия → «Войти через Google». Серверу уходит одна отметка согласия: обе обязательны
  ([docs/legal/README.md](../legal/README.md)). «Уже играю — войти» ведёт сразу ко входу; если аккаунта нет
...
- **Слои** — «Захват | Исследование» (PLAN.md, §5; tokens.md, §8): на «Захвате» тумана нет, на «Исследовании» земли
  ...
  «достижимого» (fog.md).
- **Земля** (`GameMapView`, MKMapView как у стенда S4): один `MKMultiPolygon` на стиль (отношение, цвет, уровень) без
  обводки — куски одного стиля сливаются, швов по краям тайлов нет (PLAN.md, D4). Стиль — `LandStyle`: отношение
...
- **Туман** — `MapFogRenderer`: дымка `FogStyle.haze` над дорогами, открытое вырезано маской тайла (как у стенда S4,
  ...
  сторон, минус открытое. Размытия края (σ ≈ 2 клетки) нет — только сглаживание растяжения маски.
