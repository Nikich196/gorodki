...
- **Туман** — `MapFogRenderer`: дымка `FogStyle.haze` над дорогами, открытое вырезано маской тайла (как у стенда S4,
  ...
  сторон, минус открытое. Край мягкий: маска тайла размыта один раз при сборке (σ ≈ 2 клетки,
  `FogStyle.softEdgeSigmaCells` — два прохода окна 5 клеток), у края тайла значения повторяются — шва нет.
