...
- **Вкладки** (`App/Shell`): системный `TabView` — Карта · Рейтинги · Клан · Профиль («Поиск» убран, PLAN.md, §5 и §15).
  «Карта» — земля, зоны «спорная», туман, лист участка и «Старт» ([ниже](#карта)); экран забега (HUD) — следующий
  шаг. «Рейтинги» и «Клан» пока заглушки.
...
- **Модель** — `MapModel` (`@Observable`, простые значения): слой, окраска, земля (`LandMap` из GameCore) и туман
  видимых тайлов, «сейчас», выбранный участок, шаг подсказки разрешения. Данные — через протоколы: `MapDataSource`
  (живые — `CachedMapData` поверх `TerritoryCache` и `FogCache`, фикстура — `MapFixture`), `PlayerNames`
  (`GET /players/{id}`), `RunPermissions` (система или фикстура — без настоящих запросов в снимках).
...
- **Слои** — «Захват | Исследование» (PLAN.md, §5; tokens.md, §8): на «Захвате» тумана нет, на «Исследовании» земли
  ...
  «достижимого» (fog.md).
- **Земля** (`GameMapView`, MKMapView как у стенда S4): один `MKMultiPolygon` на стиль (отношение, цвет, уровень),
  а рисует его свой `LandFillRenderer` — все куски стиля **одним путём** с правилом чёт-нечет. Стандартный
  `MKMultiPolygonRenderer` закрашивал куски по отдельности со сглаживанием, и по линии тайла внутри участка была видна
  светлая нить (снимки первых прогонов; отключить сглаживание у подкласса нельзя — свой `draw` у него MapKit не зовёт).
  Новые слои добавляются раньше, чем снимаются прежние: иначе новый объект занимал адрес только что освобождённого,
  и MapKit брал для него прежний рендерер (прежние цвета). Карта следит за моделью сама (`withObservationTracking`
  в координаторе): `updateUIView` на смену окраски не пришёл,
  и «Отношения» оставались с прежними цветами; `updateUIView` передаёт только тему и «Уменьшить движение». Стиль — `LandStyle`: отношение
  `TerritoryRelation` (своя / соперник / призрак; клана и «Ничейных земель» в ответе нет), заливка — `PlayerColor.fill`
  уровня × множитель отношения. Кромки — отдельным слоем линий (`LandEdgeRenderer`: толщина и пунктир — от отношения,
  ночью у своей — свечение тенью 3 pt).
...
- **«Старт»** — единственная цветная кнопка. Разрешение не спрошено — сначала подсказка (PLAN.md, §6.6: одна кнопка
  «Продолжить», «Всегда» не просим; тексты — по смыслу строк Info.plist): геопозиция, затем «Движение и фитнес»,
  каждая → системный запрос. Дальше, пока экрана забега нет, — «Забег — скоро».
- **Движение** — только нативное и из утверждённых приёмов (tokens.md, §7): смена слоя — стекло строки окраски
  ...
  хаптика — `.start` на «Старте», `.selection` при выборе участка. «Уменьшить движение» — всё сразу, без переходов.
...
- **Проверки.** GameCore (`LandTests`, `MomentTextTests`, Linux): отношение, уровень, щит и осада, истёкшие зоны,
  ...
  окно и подгрузка, фикстура. Снимки 24–28 (`ScreenSnapshotTests`).
...
| Аргумент | Значения |
|---|---|
| `-GorodkiScreen` | `intro`, `invite`, `age`, `consent`, `sign-in`, `map`, `leaderboards`, `clan`, `profile`, `debug`; карта с землёй и туманом фикстуры — `map-parcel` (лист своего участка с зоной «спорная»), `map-explore` («Исследование»), `map-start` (подсказка перед разрешением геопозиции) |
