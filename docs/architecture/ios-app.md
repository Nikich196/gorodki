...
Очередь пока хранится в памяти (`InMemorySyncStore`) и живёт до выгрузки приложения. Хранилище на GRDB, которое
переживает перезапуск, — отдельная задача. Его нужно передать в `AppDependencies(syncStore:)`.
