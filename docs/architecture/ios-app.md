...
- **Вкладки** (`App/Shell`): системный `TabView` — Карта · Рейтинги · Клан · Профиль («Поиск» убран, PLAN.md, §5 и §15).
  На «Карте» — подложка Apple Maps как у игры (плоская, `.muted`, без POI) и плавающий `StartButton` цвета игрока —
  единственный цветной элемент управления. «Старт» ведёт в экраны забега ([ниже](#экраны-забега)); земли и туман —
  следующие шаги; «Рейтинги» и «Клан» пока заглушки.
...
| Аргумент | Значения |
|---|---|
| `-GorodkiScreen` | `intro`, `invite`, `age`, `consent`, `sign-in`, `map`, `leaderboards`, `clan`, `profile`, `debug`; экраны забега (`RunFixture`) — `hud`, `hud-ceremony`, `hud-collapsed`, `run-result`, `run-details`, `run-history` |
| `-GorodkiFixture` | `player` — вошедший игрок из образцов `contracts/samples` (`me.json`, `fog-summary.json`, `seasons.json`); `google-ready` — кнопка Google активна; `offline`, `invite-invalid`, `google-rejected`, `account-deleting` — ошибка входа на открытом шаге |
