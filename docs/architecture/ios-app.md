...
| Очередь синхронизации | `gorodki.sqlite`, таблицы `sync*` ([sync.md](sync.md#хранилище-очереди-grdb)) | После подтверждения сервером; `wipeLocalData` — вся, с недоставленными забегами |
...
1. `RunController.resumeAtLaunch()` и `WalkLab.resumeIfNeeded()` — продолжить забег или прогулку «Лаборатории».
...
- **Live Activity.** У забега, прогулки «Лаборатории» и проверки установки один тип атрибутов, поэтому каждый хранит
  идентификатор своей плашки в UserDefaults (`LiveActivitySlot`) и после перезапуска закрывает или подхватывает только
  её. Забегу, продолженному в фоне, плашка показывается, когда приложение откроют: из фона ActivityKit её не запускает.
