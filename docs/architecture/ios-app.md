...
- **Вкладки** (`App/Shell`): системный `TabView` — Карта · Рейтинги · Клан · Профиль («Поиск» убран, PLAN.md, §5 и §15).
  ...
  ([ниже](#экраны-забега)). «Рейтинги», «Клан | Друзья | Лента» и «Входящие» —
  [ниже](#рейтинги-клан-друзья-лента-входящие).
...
## Рейтинги, клан, друзья, лента, входящие

PLAN.md, §3.4–§3.8, §3.16, §3.17, §5 (экраны 9, 11, 19, 20); адреса и образцы — контракт #149
([egor-server.md, 7.0](../guides/egor-server.md#70-заготовки-контрактов-где-что)). Код — `App/Social/`: модели
`LeaderboardsModel`, `ClanModel`, `FriendsModel`, `FeedModel`, `InboxModel` (`@Observable`, простые значения, без
SwiftUI) собраны в `SocialScreens` (`ShellModel.social`); данные — через протоколы `LeaderboardSource`, `ClanSource`,
`FriendsSource`, `FeedSource`, `InboxSource`: живые — `APISocialSource` (сгенерированный клиент), фикстуры —
`SampleSocialSource` (образцы `contracts/samples`), тесты — своя подмена.

- **Сервер за адресами — пока заглушки Егора (500).** `SocialFailure.notReady` — экран честно пишет «Пока не работает на
  сервере — скоро появится» и «Повторить»; 401 — «Нужен вход», нет связи — «Нет связи». Коды отказов из описаний адресов
  (`clan_name_taken`, `clan_join_cooldown`, `friend_code_invalid`…) — тексты `SocialFailure.message`.
- **«Рейтинги»:** «Захват | Исследование | Короли» в панели навигации; лига — меню «Бег» («Вело — с Сезона 1» выключено);
  подпись среза «Сезон 0 · срез 20.11» и пометка «итог» (`final`) или «предварительно». Строки — с устойчивым `id`
  (ник; свой ряд — `me`, одинаковые ники — с местом): при смене мест строки переезжают, числа перекатываются
  (`numericText`, `Motion.numericRoll`, при «Уменьшить движение» — сразу). Свой ряд закреплён снизу, если его нет среди
  первых (сервер отдаёт до 50) или он прокручен из виду (`onScrollVisibilityChange`). «Короли» — только «скоро».
- **«Клан»:** не в клане — вступить по коду (заглавными, как код приглашения) и «Создать свой клан» (лист, zoom из
  кнопки): название 3–24 символа — буквы, цифры, пробел, дефис (`ClanNameRule`; мат и уникальность проверяет сервер),
  оттенок — из свободных `GET /clans/hues`; после выхода до `canJoinAtMs` вступить и создать нельзя. В клане — шапка
  оттенка клана, «неполный» (меньше 3), код-приглашение лидеру и офицерам («Скопировать», `ShareLink`, «Выдать новый
  код» с вопросом), состав: лидер, офицеры, участники, роли только показываются; «Выйти из клана» с вопросом.
- **«Друзья»:** свой код («Скопировать», `ShareLink`), «Добавить по коду друга» (лист), заявки — «Принять» / «Отклонить»,
  друзья — «Удалить» с вопросом, отправленные — «Отозвать». QR-кода и сканера здесь нет: «Мой QR» и сканер делает
  листик (lab2) — подключить их к полям кода, когда он будет в main.
- **«Лента»:** «Друзья и клан | Все» (`scope=friends|all`), карточки — только поля поста: автор, дата без времени,
  «Захват +0,41 га · 41 сотка» или «Забег 5,23 км», респекты. Респект — один раз (повтор сервер не меняет): значок
  подпрыгивает (`symbolEffect(.bounce)`), лёгкая вибрация, отказ — откат. Жалоба и блокировка автора — с вопросом;
  заблокированный исчезает из ленты сразу, не дожидаясь перезагрузки. Комментариев нет (контракт).
- **«Входящие»:** колокольчик на «Карте» справа от «Старта» (стеклянная `MapGlassButton`, число непрочитанных — нейтральная
  капсула: цветной у игры только «Старт»), лист zoom-переходом; строка «Входящие» в «Профиле». Текст события — как
  прислал сервер, непрочитанные — жирным с точкой; «Прочитано» — `POST /inbox/read` до самого свежего события.

**Решено 26.09 по делегированию (Никита поручил ведущему) — можно пересмотреть:**
- «Лента» живёт во вкладке «Клан»: переключатель «Клан | Друзья | Лента»; «Входящие» — колокольчик на «Карте»
  и строка в «Профиле»;
- «Рейтинги» — «Захват | Исследование | Короли», лига, свой ряд снизу; «Короли» — заглушка «скоро» (экран «Короли
  участков» — следующая волна); «итог / предварительно» по `final`;
- «Исследование» по лиге: «Бег» — слой «Пешком» (`foot`), «Вело» — `bike`; за всё время (без `season`);
- переключатели разделов — системный сегмент в панели навигации (стекло панели рисует система); контент — без стекла;
- палитра кланов — 12 цветов палитры игроков по номеру оттенка (своих токенов у кланов нет);
- Зал славы, передача ролей, исключение и переименование клана — не в этой волне (урезано 26.09).
...
| Аргумент | Значения |
|---|---|
| `-GorodkiScreen` | `intro`, `invite`, `age`, `consent`, `sign-in`, `map`, `leaderboards`, `clan`, `profile`, `debug`; социальные (`SampleSocialSource`) — `leaderboards-exploration`, `clan-join`, `clan-create`, `friends`, `friend-add`, `feed`, `inbox`; карта с землёй и туманом фикстуры — `map-parcel` (лист своего участка с зоной «спорная»), `map-explore` («Исследование»); экраны забега (`RunFixture`) — `hud`, `hud-ceremony`, `hud-collapsed`, `run-result`, `run-details`, `run-history` |
| `-GorodkiFixture` | `player` — вошедший игрок из образцов `contracts/samples` (`me.json`, `fog-summary.json`, `seasons.json`); `player-map` — он же и карта (`MapFixture`: образцы `territory.json` и `fog.json` через те же типы API и перевод, что у живых данных, и земля вокруг — куски по тайлам UTM, все уровни, призрак, щит, осада, живая и истёкшая зоны; ники выдуманные, «сейчас» зафиксировано); `google-ready` — кнопка Google активна; `offline`, `invite-invalid`, `google-rejected`, `account-deleting` — ошибка входа на открытом шаге; `server-stub` — социальные адреса отвечают как заглушка сервера (500) |
