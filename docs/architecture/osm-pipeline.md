...
- **Проверка сборки мультиполигона на синтетическом `.osm` через osmium** — не автоматизирована (в CI `backend` osmium
  нет); на живых данных её заменяет инвариант «все отношения-площади собраны» (26.09 — собраны все, журналы ошибок пусты).
...
без набора и набор без масок дают ту же площадь). Если конфиг ссылается на незагруженный набор, заявка не решается и
повторяется (ошибка в журнале), а не судится без масок.
