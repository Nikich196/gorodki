# Конвейер OSM (`osm-pipeline` v1): проект

> Проект до кода, 24.09.2026. Кода конвейера ещё нет. Числа (ширины буферов), списки тегов и объектов OSM, границы
> Арены — на решении Никиты: [вопросы по OSM](../decisions/osm-questions.md). Здесь они — именованные параметры.
> Срок v1 — этап 2, к 04.11 (PLAN.md, §10: «`osm-pipeline` v1: маски, «достижимая» маска, границы Бреста, кварталы
> Арены»).
...
Если Никита согласится, ведущий добавит строку в [plan-deviations.md](../plan-deviations.md). Путь кода —
...
3. **Отбор по темам.** `osmium tags-filter` — отдельный файл на тему (пути, вода, ж/д, границы…). Затем
   ...
   `--show-errors` (ошибки сборки многоугольников идут в журнал, его читает шаг «Проверка») и с типом и id объекта OSM
   (`-a type,id`): по id работают «рецепты» кварталов и ручные исключения. Замкнутую линию без `area=no` osmium
   по умолчанию выдаёт и линией, и многоугольником, поэтому каждая тема берёт только свои типы геометрии (ниже, «Общие
   правила сборки»).
...
Предлагаемые таблицы. Миграция — только после слияния ветки BE-01: пока она открыта, новых миграций EF нет.
...
- **Результат** — набор `osm-set-N`. В git он **не коммитится**. Где его публиковать (релиз GitHub, страница Pages или
  артефакт CI), зависит от решения по лицензии: [вопрос 5][q5].
...
- **карта в приложении** — «© участники OpenStreetMap» в углу или за кнопкой «i», пока на карте видны слои из OSM:
  маски (если [6.9][q69] = да), граница Арены и кварталов, районы, цели «Ничейных земель». Ссылка «Правовая
...
## Маски в обработке захвата (после BE-01): только интерфейс

Файлы обработки захвата (`CaptureProcessor`, `TerritoryMap` и др.) сейчас правит ветка BE-01, поэтому здесь только
договорённость. Код — после её слияния.

Шаг A уже готов принять маски: `CaptureShapeBuilder.Build(trail, closing, Geometry? masks, settings)` вычитает их через
`GeoOps.Difference` до фильтров. Сейчас обработчик передаёт `masks: null` («маски — когда появится osm-pipeline»).
...
Только если Никита ответит «да» на [вопрос 6.9][q69]. Сейчас этого нет ни на сервере, ни в контракте, ни на телефоне.
...
- **Стиль** — решение Никиты по дизайну. Штриховка занята землёй клана (§6.3: «клан — штриховка»); в вопросе 6.9
  предложена серая заливка.

## Проверка (когда v1 готов)
...
- **Проверка сборки OSM** — там, где есть osmium (ноутбук или `osm-pipeline.yml`, не CI `backend`): синтетический
  ...
  целиком.
- **Инвариант набора: все отношения-площади собраны.** Каждое отношение-площадь темы (после `tags-filter`) есть в
  выходе `osmium export` многоугольником (сверка id на входе и выходе), и журнал `--show-errors` по ним пуст. Не
  собравшееся отношение с точками в рамке города — набор не выпускается. Исключение — только явным списком id в
  параметрах, с причиной, и только для отношений вне города. Границы берутся не фильтром по тегам, а по id (город и
  страна), поэтому соседние страны, чьи отношения в выгрузке Беларуси заведомо неполны, в проверку не попадают.
