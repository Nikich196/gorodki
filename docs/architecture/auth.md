...
Открытые адреса утверждены списком в тесте `AnonymousAccessTests`: проверки здоровья (`/health`, `/health/ready`) и вход
(`/auth/*`), в разработке — ещё описание API и страница Scalar. Тест берёт адреса у сервера и решает за каждый, как
`AuthorizationMiddleware` (`AllowAnonymous`, политики адреса, политика по умолчанию), а каждому закрытому шлёт запрос
без токена и ждёт 401. Новый открытый адрес или потерянная политика «по умолчанию — только с входом» роняют тест. Хаб
реального времени закрыт своим `[Authorize]`, а не только политикой по умолчанию.
...
| `GET /me/stats` | Своя статистика: забеги, засчитанные метры, открытая площадь, место в рейтинге — только числа, без площади земли. **Задача #116 (Егор)**: пока заглушка | 401 без токена |
| `GET /players/{id}` | Карточка игрока: ник (с его согласия или свой) или «Игрок #1234», цвет. **Задача #115 (Егор)**: пока заглушка | `player_not_found` (404 — нет или удаляется), 401 без токена |
...
| `GET /me/weekly`, `GET /me/streak`, `GET /me/inventory`, `POST /me/inventory/{id}/activate`, `GET /me/blocks`, `PUT/DELETE /me/blocks/{id}` | Карточка недели, серия, Рюкзак, блокировки. **Задачи #141, #148, #140, #144 (Егор)**: пока заглушки; коды ошибок — в описании API, остальные адреса заготовок (кланы, друзья, лента, «Входящие», «Коллекция», отрезки, дуэли, рейтинг территории, Зал славы) — [egor-server.md, 7.0](../guides/egor-server.md#70-заготовки-контрактов-где-что) | 401 без токена; чужая фишка — 404 `item_not_found` |
| `POST /admin/invites`, `GET /admin/invites`, `DELETE /admin/invites/{code}` | Админ: выпустить инвайт-коды (`XXXX-XXXX`), список с числом использований, погасить код. Не входят в описание API для приложения. **Задача #114 (Егор)**: пока заглушки | `admin_only` (403), `invites_invalid` (400), `invite_not_found` (404) |
...
- **Истёкшие токены стираются** раз в час (задача Hangfire `refresh-tokens-purge`, [jobs.md](jobs.md);
  `RefreshTokenRetention`): каждое обновление добавляет строку, и без чистки таблица и «мои данные» росли бы вечно.
  Заменённые, но не истёкшие остаются до конца срока — по ним узнаётся повтор украденного токена.
...
- `GET /me/export` — JSON-файл (`Content-Disposition: attachment`): профиль и согласия, входы (когда выдан, срок, отозван;
  ...
  сезонам и тайлам (биты — как в `GET /fog`), приватные зоны, места в ежедневных срезах рейтингов (хранятся неделю) и
  очки сезона — только уже видимые начисления (у захвата — с границы публичности, как и земля;
  [очки и сезоны](scoring-and-seasons.md#приватность-очков)).
