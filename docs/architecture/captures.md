...
- **Когда**: один раз за забег, в том же фоновом обработчике, что и захваты (после них, перед туманом), когда забег
  ...
  только что пробежал здесь»; у демо-аккаунта — сразу. Если в тайлах забега ещё скрыт захват земли игрока — позже
...
- **Пока скрыт захват земли игрока** (BE-01): визиты забега не засчитываются, пока в тайлах его пути (прямоугольник
  тайлов вокруг засчитанного пути) есть ещё скрытый от всех захват (`TerritoryReader.HiddenJournal` — то же, что у
  карты), в журнале которого — до или после — есть земля этого игрока. Иначе у жертвы они легли бы на треснувшую часть
  и остаток куска, у автора (захват применён позже конца забега — забег из офлайна) — на взятое и освежённое, у каждой
  части своё время визита и свой порог 50 м, и публичная проекция выдала бы скрытую петлю швом
  ([territory-map.md](territory-map.md)). После раскрытия визиты ложатся на публичные куски. Цена — задержка визита
  до 25 минут сверх обычных 20, только у таких забегов. Это не ошибка: `visits_failures` не растёт, `visits_retry_at`
  ставится на момент раскрытия (`TerritoryReader.PublicAt`), и до него забег не берётся в очередь. Ожидание конечно:
  захват раскрывается не позже чем через задержку и шаг раскрытия после применения.
