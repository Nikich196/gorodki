...
- **Визиты владельцев** касанием не считаются (`replayVisits`; откат нарушителя и публичная проекция скрытого захвата):
  ...
  (визиты автора на взятом). Без этого режима «тронутая» земля — любая изменённая (так проверяет растровый оракул).
- Проверено: откат последнего захвата возвращает прежнюю карту (property-тест), откат захвата из середины истории
  ...
  владельцев — с растровым оракулом переноса визитов, инварианты карты после отката.
...
7. Свои касания нарушителя (визиты, повторные петли по отнятой земле) «тронутой» её **не делают**: иначе, побегав по
   украденному, он бы его «отмыл». Визиты жертвы — тоже: треснувшая часть возвращается целой, с этим визитом. Касание
   чужого — захват, смена сезона — по-прежнему защищает землю.
...
- **Пока чужой захват скрыт**: конец забега бывает публичен раньше захвата из того же забега (забег из офлайна), и визит
  ложится на землю скрытого захвата — автора на взятое, жертвы на треснувшую часть. Публичная проекция переносит такие
  визиты на прежнюю землю, и захват не раскрывается раньше времени ([territory-map.md](territory-map.md)).
