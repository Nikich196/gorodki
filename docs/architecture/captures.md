...
(`CaptureContext.BigLoop`), признак считает `CaptureProcessor` перед движком.
...
- **Чужая земля внутри** — и L1, и L2/L3 — не переходит и не трескается: ни щита, ни осады, окно и счётчики снятия уровней
  не трогаются. Только пометка «спорная» `contested_until = время петли + territory.contestedHours` (24 ч) — у той части
  куска, что внутри петли. В итоге заявки это `contested` в `areaByOutcome`; во «взятое» не входит.
- **Всё остальное — как у обычной петли**: ничья и угасшая земля берётся, своя освежается, соклановцев — освежается им;
  щит, опоздавшая петля (`superseded`) и новый аккаунт (`newAccountLimited`) решаются раньше и пометку не ставят.
- **Пометка без игровой силы**: не мешает ни визиту и росту уровня, ни обычной петле (L1 с пометкой переходит как обычно);
  визит её не снимает. Более поздняя большая петля продлевает её, пришедшая с опозданием — не укорачивает.
- **Приватность**: пометка — изменение чужой земли, поэтому пишется в журнал (`capture_journal_pieces.contested_until`) и
  раскрывается вместе с захватом — через 20–25 минут всем, включая владельца (публичная проекция откатывает её, как
  любой скрытый захват). Откат нарушителя её снимает. На карте — `contestedUntilMs`, вверх до 10 минут у всех, кроме
  администратора: минута петли не видна и владельцу ([territory-map.md](territory-map.md)).
- Проверено: `CaptureRulesTests` (правила куска), `TerritoryMapScenarioTests.Big_loop_…`/`Loop_just_under_the_threshold_…`
  (0,56 и 0,49 км² на карте, откат), property-тесты гоняют каждую 4-ю (инварианты) и 3-ю (откат) петлю как большую,
  `TerritoryTests.Big_loop_only_marks_enemy_land_contested_…` (база, скрытие до границы, округление).
