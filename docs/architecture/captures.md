...
flowchart TD
    ...
    R --> A1["Шаг A: контур P минус маски набора<br/>osm.setVersion (IMaskStore), вне транзакций"]
...
- **Изоляция**: судья зовётся после аренды первой заявки. Непредвиденная ошибка заявки (испорченный кусок, сбой базы)
  пишется в `last_error`, аренда остаётся паузой до повтора; после 5 аренд — `failed` с `too_many_attempts`. Так же
  кончается заявка, если конфиг забега ссылается на незагруженный набор масок OSM: `MaskStore` бросает
  `InvalidOperationException`, и без масок заявка не судится
  ([osm-pipeline.md](osm-pipeline.md#маски-в-обработке-захвата)). Каждая стадия прохода обработчика (захваты, визиты,
  туман, раскрытия, откаты) и каждый забег в ней — отдельно: ошибка одного не останавливает остальное. Забег, у которого
  не посчитались туман или визиты, откладывается (`fog_retry_at`, `visits_retry_at`: 1, 2, 4… минуты, не реже раза в
  час) и уходит из очереди сам, когда стираются его точки. Визиты, которые ждут раскрытия скрытого захвата
  ([ниже](#визиты)), откладываются тем же `visits_retry_at` до момента раскрытия — это не ошибка, счётчик не растёт.
...
- **Очки за захват** (§3.5) — строка книги очков `score_events` в той же транзакции, под блокировкой игрока (ступени
  суток считаются по уже начисленному за сутки); сезон и сутки — по времени петли; другим (и самому автору) видны с
  границы публичности применения, как разбивка итога. Петля, которая ничего не взяла (только освежила свою землю),
  очков не даёт. Как считается — [очки и сезоны](scoring-and-seasons.md#захват).
...
- **Откат** (`Restore`): внутри следа там, где земля **и сейчас такая, какой её оставил захват**, возвращается прежнее
  состояние; где её с тех пор изменили — другой захват, удаление игрока, другой откат, — всё остаётся как есть. Смена
  сезона землю «тронутой» не делает: мягкий сброс той же формулой меняет и журнал (землю «до» и «после»), поэтому откат
  через смену сезона возвращает сброшенную землю ([очки и сезоны](scoring-and-seasons.md#как-выполняется)); после
  чистой карты Сезона 0 журнала нет.
...
5. Захват остаётся со статусом `applied` и получает `rolled_back_at`: для суточного лимита он по-прежнему считается,
   ...
   Его очки (`score_events`) стираются в той же транзакции.
...
7. Свои касания нарушителя (визиты, повторные петли по отнятой земле) «тронутой» её **не делают**: иначе, побегав по
   ...
   петлёй — от визита его не отличить. Касание чужого — захват (кроме такого освежения) — по-прежнему защищает землю.
   Смена сезона — нет: журнал сбрасывается вместе с землёй, и жертва получает свою землю уже сброшенной.
...
- **Что делает**: закрепляет действующий уровень (угасание начинается заново) и +1, если прошло ≥ 20 ч с прошлого
  ...
  Время визита назад не сдвигается (забег, пришедший с опозданием, не «старит» кусок). Визит — «касание владельца»
  (`touched_at`, «касались в этом сезоне» для удержания, [очки и сезоны](scoring-and-seasons.md#касались-в-этом-сезоне--touched_at)).
  Визит до полуночи смены сезона, посчитанный уже после смены (забег кончился после ~23:35), не поднимает уровень
  сброшенной земли: изменённый им кусок проходит тот же мягкий сброс — как в порядке «визит, потом сброс»; так же и петля
  до полуночи, применённая после смены ([очки и сезоны](scoring-and-seasons.md#как-выполняется)).
- **Дистанция** для очков (+10 за км до 20 км в сутки, §3.5) считается здесь же, в той же транзакции: путь без первых и
  последних 200 м, под блокировкой игрока (суточный потолок), один раз за забег.
...
| `season_wipe` (итог `stale`) | Петля замкнута до старта сезона с чистой карты (С0), а применяется после смены: земли полевых тестов больше нет ([очки и сезоны](scoring-and-seasons.md#старт-сезона-0--чистая-карта)) |
...
без него, [PLAN §7.3](../PLAN.md#73-бэкенд)), показ масок OSM на карте ([osm-pipeline.md](osm-pipeline.md#маски-на-карте)),
кланы. Маски в шаге A уже вычитаются (26.09, [osm-pipeline.md](osm-pipeline.md#маски-в-обработке-захвата)).
