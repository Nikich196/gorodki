...
- **Строки точного отката** (`capture_journal_parcels`, аудит BE-01): какие строки `parcels` захват удалил (с контуром в
  ...
  число таких тайлов. Хранятся `max(3 ч, 2 × (задержка + шаг раскрытия))` — стираются той же часовой чисткой, живут до
  четырёх часов. Три часа, а не «вдвое от задержки», — запас на подъём задержки в конфиге: срок считается по задержке на
  момент чистки, а скрытость — на момент чтения, и до 2 ч 55 мин новой задержки строки снова скрытых захватов ещё лежат.
  Размер таблицы — в `/health/ready` (`captureJournalParcelsBytes`). Проекция читает журнал тремя запросами на всё
  чтение карты, сколько бы скрытых захватов в нём ни было (`CaptureJournal.LoadHiddenAsync`).
