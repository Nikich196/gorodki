...
/// стартовый экран с «Проверкой установки» и «Лаборатория» — прогулка (S1), стенд карты (S4), сервер (S7). Идут
/// в ios-snapshots.yml на симуляторе iPhone; снимки — вложения XCTest, workflow выгружает их из пакета результатов
/// в PNG (артефакт запуска): так приложение можно посмотреть без Mac.
///
...
final class ScreenSnapshotTests: XCTestCase {
    ...
    /// не выполнен — как на пробной установке до экрана входа. С панелью S7 здесь же видно «нужен вход» в разделе
    /// «Реальное время».
    @MainActor
