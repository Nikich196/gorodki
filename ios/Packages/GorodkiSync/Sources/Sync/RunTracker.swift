...
public actor RunTracker {
    ...
    /// «Запаздывание…» в сводке «Лаборатории»: получение минус время записи); до замера — 10 секунд. Не хватит запаса —
    /// опоздавшие записи в забеге посчитает `RunRecorder.lateSensorRecords`.
