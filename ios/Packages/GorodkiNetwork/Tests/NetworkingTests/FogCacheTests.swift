...
struct FogCacheTests {
    ...
    /// в базе нет: спрошенный с `@0` он «не изменился», без версии — не упоминается вовсе. Стёртый очисткой истории,
    /// спрошенный с версией, приходит пустым с версией на 1 больше.
    actor Server: ClientTransport {
        private var versions: [FogTileRef: Int64] = [:]
        private var cleared: Set<FogTileRef> = []
        ...
        func set(_ key: FogTileRef, version: Int64) {
            versions[key] = version
            cleared.remove(key)
        }
        /// «Очистить историю исследований»: строки тайла больше нет.
        func clear(_ key: FogTileRef) {
            versions[key] = nil
            cleared.insert(key)
        }
        ...
        func send(
            ...
            for item in asked {
                ...
                } else if cleared.contains(key), let known, known > 0 {
    ...
    @Test("История очищена — пустой тайл с версией новее заменяет стёртый; открытый заново приходит")
    func clearedHistoryReplacesCachedTiles() async throws {
        ...
        #expect(empty.version == 4 && empty.cellCount == 0 && FogTileCodec.cellCount(empty.words) == 0)
        #expect(await cache.tile(Self.b)?.cellCount == 10)  // другой тайл не тронут
        #expect(await cache.corruptedTiles == 0)

        await server.set(Self.a, version: 5)  // новый забег открыл его снова — версия больше пустой
        await cache.invalidate()
        #expect(try await cache.refresh(visible: Self.near) == [Self.a])
        #expect(await cache.tile(Self.a)?.cellCount == 50)
    }
