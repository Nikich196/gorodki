...
/// «туман изменился» и после переподключения к нему (подсказки за время разрыва потеряны). Версии свои у каждого
/// игрока — при смене аккаунта кэш очищается (`reset`). Повреждённый тайл пропускается и считается
