...
public struct GRDBSyncStore: SyncStore {
    ...
    /// стирается вместе с остальными, когда забег подтверждён или отвергнут (`deleteChunks`), — если сам забег читается:
    /// нечитаемый забег остаётся в таблице со всеми кусками и заявками. Номер нечитаемой заявки новая не займёт
    /// (`lastClaimNo`).
