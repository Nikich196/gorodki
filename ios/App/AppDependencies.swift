...
final class AppDependencies: Sendable {
    ...
    /// Сколько забегов игрока ещё не дошли до сервера целиком — их сотрёт выход (`wipeLocalData`). Пробные забеги
    /// «Лаборатории» и забеги, в которых сервер окончательно отказал, не считаются: отправлять из них нечего.
    func unsentRunCount() async -> Int {
        let runs = (try? await syncStore.runs()) ?? []
        return runs.filter { !$0.isLabProbe && !$0.confirmedComplete && $0.serverState != .rejected }.count
    }
