// Каждое имя SF Symbols из кода приложения, расширения и GorodkiKit есть в системе (PLAN.md, §13, «Автоматически
// (PR)»: «iOS: сборка Debug и Release, локализация, SF Symbols»).
// Опечатка в имени — не ошибка сборки: `Image(systemName:)` молча рисует пустое место, и заметно это только глазами.
// Задача sf-symbols в ios-snapshots.yml — автоматическая в PR, но пока не обязательная для слияния
// (docs/plan-deviations.md, п. 7). Раннер macos-26: символы берутся у NSImage той же 26-й системы, что
